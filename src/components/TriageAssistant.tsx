  ...
  async function sendMessage(e: React.FormEvent) {
    ...
    try {
      ...
      setMessages((prev) => [
        ...
        {
          role: "assistant",
          content: `I ran into an issue: ${error?.message || "unknown error"}. Double-check the admin logs or AI service configuration.`,
        },
      ]);
    } finally {
      setLoading(false);
    ...
    <div className="mx-auto flex w-full max-w-4xl flex-col gap-4">
      <header className="space-y-2 rounded-2xl border border-white/20 bg-white/10 p-5 text-white shadow-lg shadow-cyan-500/10">
        ...
        <p className="text-sm text-white/75">
          Capture presenting concerns, red flags, and care barriers. Responses are generated by the configured AI service and should be reviewed before sharing with community members.
        </p>
      </header>
      ...
      <div className="glass-panel relative flex min-h-[380px] flex-col gap-4 p-4 text-slate-900 sm:p-6">
        <div className="flex-1 space-y-4 overflow-y-auto pr-1">
          {messages.map((message, idx) => {
            const base = "max-w-lg rounded-2xl px-4 py-3 text-sm leading-relaxed shadow whitespace-pre-line";
            const variant =
