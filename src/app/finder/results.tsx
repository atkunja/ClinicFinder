      ...
      <div className="app-surface flex h-full flex-col overflow-hidden text-slate-900 shadow-xl shadow-slate-900/10">
        <div className="border-b border-slate-200/50 bg-slate-50/70 px-5 py-4 sm:px-6">
          <div className="flex flex-wrap items-center gap-3">
            <h2 className="text-lg font-semibold text-slate-900 sm:text-xl">Clinic matches</h2>
            <span className="inline-flex items-center gap-1 rounded-full bg-slate-200/80 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.25em] text-slate-600">
              {clinics.length} {clinics.length === 1 ? "result" : "results"}
            </span>
          </div>
          <p className="mt-2 text-xs uppercase tracking-[0.35em] text-slate-400">Sorted by distance</p>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-5 sm:px-6">
          {clinics.length === 0 ? (
            <div className="rounded-2xl border border-dashed border-slate-300/90 bg-slate-50/90 px-5 py-6 text-sm text-slate-600">
              <h3 className="text-base font-semibold text-slate-800">No clinics matched just yet</h3>
              <ul className="mt-3 list-disc space-y-1 pl-5 text-xs text-slate-500">
                <li>
                  Toggle off <span className="font-semibold text-slate-700">Verified only</span> to explore every option.
                </li>
                <li>Try a broader radius or search with a nearby landmark.</li>
                <li>
                  Need hands-on help? Email <span className="font-semibold text-slate-700">hello@zbimpact.org</span> and
                  our navigators will guide you.
                </li>
              </ul>
            </div>
          ) : (
                      ...
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        {clinic.miles !== undefined && isFinite(clinic.miles) && (
                          <span className="inline-flex items-center gap-1 rounded-full bg-emerald-50 px-2.5 py-1 text-xs font-medium text-emerald-700">
                            {(clinic.miles as number).toFixed(clinic.miles < 10 ? 1 : 0)} mi
                          </span>
                        )}
                        {clinic.verified && (
