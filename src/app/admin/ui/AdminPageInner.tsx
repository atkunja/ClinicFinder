...
function toRow(d: QueryDocumentSnapshot<DocumentData>): ClinicRow {
  const data = d.data() as Partial<ClinicDoc> & { coords?: Coords; lat?: number; lng?: number };
  const fixed = fixCoords(data?.coords?.[0] ?? data?.lat, data?.coords?.[1] ?? data?.lng);
  return {
    docId: d.id,
...
export default function AdminPageInner() {
  ...
  function startAdd() {
    setEditing(null);
    setForm({ docId: "", id: "", name: "", address: "", url: "", services: [], coords: [0, 0], summary: "" });
  }
  ...
  async function handleSave() {
    ...
    try {
      ...
      const services = Array.isArray(latest.services)
        ? latest.services
        : String(latest.services ?? "").split(",").map((s) => s.trim()).filter(Boolean);
  ...
  async function handleGenerateSummary() {
    ...
    if (!url) { setMsg("Enter the clinic website URL first."); return; }
    setBusy(true);
  ...
  return (
    <div className="max-w-5xl mx-auto p-6 text-black">
      <div className="flex items-center justify-between mb-4">
        ...
        <Link href="/finder" className="text-emerald-700 underline">← Back to Finder</Link>
      </div>

      {msg && <div className="mb-3 text-sm text-emerald-700">{msg}</div>}

      <div className="rounded border bg-white p-4 mb-8">
        <h2 className="font-semibold mb-3">{editing ? "Edit Clinic" : "Add New Clinic"}</h2>

        <div className="grid md:grid-cols-2 gap-3">
          <input className="border p-2 rounded" placeholder="Document ID (used as slug)"
                 value={form.id} onChange={(e) => setField("id", e.target.value)} />
          <input className="border p-2 rounded" placeholder="Name"
                 value={form.name} onChange={(e) => setField("name", e.target.value)} />
          <input className="border p-2 rounded md:col-span-2" placeholder="Street address"
                 value={form.address} onChange={(e) => setField("address", e.target.value)}
                 onBlur={async (e) => { const c = await geocodeAddress(e.currentTarget.value); if (c) setField("coords", c); }} />
          <input className="border p-2 rounded md:col-span-2" placeholder="Website URL"
                 value={form.url} onChange={(e) => setField("url", e.target.value)} />
          <input className="border p-2 rounded md:col-span-2" placeholder="Services (comma-separated)"
                 value={form.services.join(", ")}
                 onChange={(e) => setField("services", e.target.value.split(",").map((s) => s.trim()))} />
          <input className="border p-2 rounded" placeholder="Lat"
                 value={form.coords[0]} onChange={(e) => setField("coords", [Number(e.target.value) || 0, form.coords[1]])} />
          <input className="border p-2 rounded" placeholder="Lng"
                 value={form.coords[1]} onChange={(e) => setField("coords", [form.coords[0], Number(e.target.value) || 0])} />
        </div>

        <label className="block text-sm mt-4 mb-1">Summary</label>
        <textarea className="border rounded w-full p-2 min-h-[140px]"
        ...
        <div className="flex gap-3 mt-4">
          ...
          <button disabled={busy || !form.url} onClick={handleGenerateSummary}
                  className="px-4 py-2 rounded bg-indigo-600 text-white disabled:opacity-60"
                  title={!form.url ? "Enter the clinic website URL first" : ""}>
            {busy ? "Fetching…" : "Generate Summary"}
          </button>
          <button onClick={startAdd} className="px-4 py-2 rounded border">Cancel</button>
        </div>
      </div>
          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.docId} className="border-b">
                ...
                <td className="px-3 py-2 truncate max-w-[260px]">{r.summary ? r.summary.slice(0, 80) : "—"}</td>
                <td className="px-3 py-2 flex gap-2">
                  <button className="px-2 py-1 border rounded" onClick={() => startEdit(r)}>Edit</button>
                  <a className="px-2 py-1 border rounded" href={r.url} target="_blank" rel="noreferrer">Website</a>
                  <button className="px-2 py-1 border rounded text-red-600" onClick={() => handleDelete(r.docId)}>Delete</button>
                </td>
              </tr>
            ))}
            {!rows.length && (
              <tr><td className="px-3 py-4 text-gray-600" colSpan={5}>No clinics yet.</td></tr>
            )}
          </tbody>
        </table>
      </div>
