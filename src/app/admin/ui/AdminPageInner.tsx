...
export default function AdminPageInner() {
  ...
  async function geocodeAddress(q?: string) {
    ...
    try {
      ...
      const newLat = String(parseFloat(data[0].lat));
      const newLng = String(parseFloat(data[0].lon));
  ...
  async function autofillFromWebsite() {
    ...
    try {
      ...
      setHours({
        Mon: h?.Mon || h?.Monday || "",
        Tue: h?.Tue || h?.Tuesday || "",
        Wed: h?.Wed || h?.Wednesday || "",
        Thu: h?.Thu || h?.Thursday || "",
        Fri: h?.Fri || h?.Friday || "",
        Sat: h?.Sat || h?.Saturday || "",
        Sun: h?.Sun || h?.Sunday || "",
      });

      // coords → if missing or zero, geocode from address
      const hasCoords = Array.isArray(d.coords) && Number.isFinite(+d.coords[0]) && Number.isFinite(+d.coords[1]);
      if (hasCoords) {
        setLat(String(d.coords[0])); setLng(String(d.coords[1]));
      } else if (d.address) {
  ...
  async function saveClinic(e: React.FormEvent) {
    ...
    try {
      ...
      // ensure coords are valid; if not, try geocoding
      let la = parseFloat(lat), lo = parseFloat(lng);
      if (!Number.isFinite(la) || !Number.isFinite(lo) || (la === 0 && lo === 0)) {
        const g = await geocodeAddress(address);
        la = parseFloat(g?.[0] ?? "NaN"); lo = parseFloat(g?.[1] ?? "NaN");
      }
    ...
    <div className="admin-scope max-w-4xl mx-auto px-4 py-6 text-black">
      ...
      <div className="mb-6 space-y-2 rounded-xl border border-emerald-200/60 bg-emerald-50/70 p-4 text-emerald-900">
        ...
        <p className="text-sm text-emerald-800/80">
          Capture presenting concerns, urgency flags, and barriers before scheduling. Open to any volunteer account and powered by your configured AI service—review responses before sharing with patients.
        </p>
        <Link
          href="/triage"
      ...
      <form onSubmit={saveClinic} className="rounded-lg border p-4 space-y-3 bg-white text-black">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <input className="border rounded px-3 py-2 bg-white text-black placeholder:text-gray-500"
            placeholder="Document ID (used as slug)" value={id} onChange={(e)=>setId(e.target.value)} />
          <input className="border rounded px-3 py-2 bg-white text-black placeholder:text-gray-500"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <input className="border rounded px-3 py-2 bg-white text-black placeholder:text-gray-500"
            placeholder="Website URL" value={url} onChange={(e)=>setUrl(e.target.value)} />
        ...
        <input className="border rounded px-3 py-2 w-full bg-white text-black placeholder:text-gray-500"
          placeholder="Services (comma-separated)" value={servicesInput} onChange={(e)=>setServicesInput(e.target.value)} />

        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <input className="border rounded px-3 py-2 bg-white text-black placeholder:text-gray-500"
            placeholder="Languages (comma-separated)" value={languagesInput} onChange={(e)=>setLanguagesInput(e.target.value)} />
          <input className="border rounded px-3 py-2 bg-white text-black placeholder:text-gray-500"
            placeholder="Eligibility (comma-separated)" value={eligibilityInput} onChange={(e)=>setEligibilityInput(e.target.value)} />
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          <div className="flex gap-2">
            <input className="border rounded px-3 py-2 bg-white text-black placeholder:text-gray-500"
              placeholder="Latitude" value={lat} onChange={(e)=>setLat(e.target.value)} />
            <input className="border rounded px-3 py-2 bg-white text-black placeholder:text-gray-500"
        ...
        <div className="grid grid-cols-2 md:grid-cols-7 gap-2">
          {DAYS.map(d => (
            <div key={d} className="flex flex-col">
              ...
              <input className="border rounded px-2 py-1 text-sm bg-white text-black placeholder:text-gray-500"
        ...
        <div className="flex gap-2">
          ...
          <button type="button"
            onClick={() => setSummary(buildSummary({ servicesInput, languagesInput, eligibilityInput }))}
            className="px-3 py-2 border rounded bg-white text-black">
            Generate Summary
          </button>
          <button type="button" onClick={resetForm} className="px-3 py-2 border rounded bg-white text-black">
            Cancel
          </button>
        </div>
      </form>
